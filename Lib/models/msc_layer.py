...
class MatrixSparseCodingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        c, weight = self.fista(x.view(-1, dim))

        # Compute loss
        xp = F.linear(c, weight.T, bias=None)
        r = x.view(-1, dim).repeat(1, self.n_dict) - xp
        r_loss = torch.sum(torch.pow(r, 2)) / self.n_dict
        c_loss = self.lmbd * torch.sum(torch.abs(c)) + self.mu / 2. * torch.sum(torch.pow(c, 2))

        return c.view(bs, token, -1), xp.view(bs, token, -1), r, (r_loss, c_loss)
...
if __name__ == '__main__':
    ...
    input_channel = 3
    output_channel = 64
    layer1 = MatrixSparseCodingLayer(n_channel=input_channel, dict_size=output_channel)

    y = layer1(torch.randn(1, 512))
